...
bool Boundary::CheckCollision(GameObject* object, Camera* player)
{
	...
	PlayerMax.Set(float(player->position.x + (CameraScale.x * 0.5f)), float(player->position.y + (CameraScale.y * 0.5f)), float(player->position.z + (CameraScale.z * 0.5f)));
	PlayerMin.Set(float(player->position.x - (CameraScale.x * 0.5f)), float(player->position.y - (CameraScale.y * 0.5f)), float(player->position.z - (CameraScale.z * 0.5f)));
